...
// Get all products for seller dashboard - marketplace view
router.get('/all-products', async (req: Request, res: Response) => {
  try {
    console.log('[/api/sitedata/all-products] Request received');
    
    // FIX: Ensure seller_id is cast to INTEGER for proper type matching
    const query = `
      SELECT 
        p.productid,
        p.title,
        ...
        COALESCE(s.name, 'Marketplace') as seller_name,
        COALESCE(s.storename, 'Marketplace') as storename
      FROM products p
    ...
    const response = await client.query(query);
    console.log(`[/api/sitedata/all-products] Success - Returned ${response.rows.length} products`);
    
    if (response.rows.length > 0) {
      console.log('[/api/sitedata/all-products] Sample products:', {
        productid: response.rows[0].productid,
        title: response.rows[0].title,
        seller_id: response.rows[0].seller_id,
        seller_name: response.rows[0].seller_name
      });
    }
    
    res.status(200).json({ data: response.rows });
  } catch (error) {
    console.error('[/api/sitedata/all-products] Database error:', error);

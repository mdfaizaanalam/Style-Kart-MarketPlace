...
router.post('/user/orders', userTokenSchema, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        const { userIDToken } = matchedData(req);
        // ✅ REPLACE EXISTING QUERY WITH THIS
        const query = `
  SELECT
      o.orderid,
      o.totalamount,
  ...
  INNER JOIN
      shipping s ON oi.shippingid = s.shippingid
  INNER JOIN
      payments pm ON oi.paymentid = pm.paymentid
  LEFT JOIN
      return_requests rr ON o.orderid = rr.order_id AND p.productid = rr.product_id
  WHERE
      o.userid = $1 AND pi.isprimary = true
  ORDER BY
      o.createdat DESC;
`;
        try {
...
router.get('/user/order-detail/:userIDToken/:orderID', orderSchema, async (req: Request, res: Response) => {
    ...
    if (result.isEmpty()) {
        ...
        orders.orderid,
        orders.createdat,
        shipping.deliveredat as delivered_date, 
        ...
        orders.order_code,
        orders.totalamount,
        orders.delivered_date as order_delivered_date
        FROM orders
